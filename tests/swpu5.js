...
console.log('\nДүнзний ПУ-5 маягт (exportSwPu5Book)');
{
  ...
  // ── Нийт дүн (эх/frozen тооцоотой, дэрийнхтэй ижил зарчим) ──
  const ov=wb.getWorksheet('Нийт дүн');
  const orow=V(ov.getRow(3)).slice(1);
  ok('Нийт дүн: сум7, дэр 3/1, дүнз 10/31, тэнцэхгүй 6/18,5',
     orow[0]===7&&orow[2]===3&&orow[3]===1&&orow[4]===10&&orow[5]==='31'
     &&orow[6]===6&&orow[7]==='18,5',JSON.stringify(orow));
  ok('Хувь ойролцоогоор 59,7%',Math.abs(orow[8]-18.5/31)<1e-9,String(orow[8]));
  ...
  ok('Д/Д: Дэр 1-3, 3п/м 1-6, 3,25 1-4, НИЙТ 10',
     dd[0]==='Д/Д'&&dd[1]==='1-3'&&dd[2]==='1-6'&&dd[3]==='1-4'&&dd[4]===10,JSON.stringify(dd));
  ok('Пог/м: Дэр хоосон, 3п/м 18, 3,25 13, НИЙТ 31',
     pm[1]===''&&pm[2]==='18'&&pm[3]==='13'&&pm[4]==='31',JSON.stringify(pm));
  ok('Тэнцэхгүй /ш: Дэр 1, 3п/м 4, 3,25 2, НИЙТ 6',
     bs[1]===1&&bs[2]===4&&bs[3]===2&&bs[4]===6,JSON.stringify(bs));
  ok('Тэнцэхгүй пог/м: Дэр хоосон, 3п/м 12, 3,25 6,5, НИЙТ 18,5',
     bm[1]===''&&bm[2]==='12'&&bm[3]==='6,5'&&bm[4]==='18,5',JSON.stringify(bm));
}
